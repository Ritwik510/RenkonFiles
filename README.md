...
**No. 1**: The database contains 2 files, one for the exchange rates and other for the names of the currencies that it supports. In the beginning, Renkon can ask the user to type in the amount (call it M) of money that has to be used in basic calculations that follow up. Then, Renkon can ask the user to enter the currency of the amount that has just been entered and then proceed to ask the user, the currency in which M should be converted to (the information has to be typed in). If the exchange rate is available in the database for particular choice of currencies, the user is provided with the result (along with the time and date of the latest update of market rates) and if not, the user is conveyed the same. If the user wants to convert an amount again, he/she can do so by giving a ‘yes’ to the ‘Convert again?’ question asked in the end. Thus, the termination of the program lies in the hand of the user.


**No. 2**:The database contains 2 files, one for the exchange rates and other for the names of the currencies that it supports. To save space, the first file will be containing the exchange rates of 15 currencies with respect to United States $ only. Initially, Renkon displays the list of all the currencies that it supports and a corresponding currency number for each. The user has to input the amount and then enter the currency number to state the currency in which the amount was entered.
Then, Renkon asks for the currency number, to know which currency to convert to , after which it automatically converts the amount using the exchange rate that has beed notified to it by the user (Two computations are performed internally, one, conversion from first currency to $ and then conversion from $ to the desired currency). The result is then displayed on the screen along with the time and date of the latest update of market rates.If the user wants to convert an amount again, he/she can do so by giving a ‘yes’ to the ‘Convert again?’ question asked in the end. Thus, the termination of the program lies in the hand of the user.
